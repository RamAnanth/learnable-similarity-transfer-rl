# DQN with explicit Shaping: Code for DQN adapted from PyTorch tutorial on DQN
import math
import random
import numpy as np
...
class DQN_Reshaped(object):
    # Class that defines the algorithm for DQN with static advice using explicit Shaping
    ...
    def learn_phi(self, transition_tuple):
        ...
        if self.transfer_mode=='delta_action':
            ...
            state_phi = self.phi(state_batch).squeeze(1)
            next_state_phi[non_final_mask] = self.phi(non_final_next_states).squeeze(1).detach()
        ...
        if self.transfer_mode == 'advantage':
    ...
    def select_action(self,state):
        # Select the action to be used by the agent
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                if self.similarity == 'delta':
                    max_delta = torch.max(np.abs(self.delta(state).detach())) # Scale with value function
                    bias_decay = self.kappa*np.exp(-max_delta)
                else:
    ...
    def select_greedy_action(self,state):
        ...
        with torch.no_grad():
            if self.similarity == 'delta':
                max_delta = torch.max(np.abs(self.delta(state).detach()))
                bias_decay = self.kappa*np.exp(-max_delta)
            else:
                bias_decay = self.kappa
            return (self.policy_net(state) - bias_decay*bias).max(0)[1].view(1, 1)
